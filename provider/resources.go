...
import (
	...
	shimprovider "github.com/devopsarr/terraform-provider-readarr/v2/shim"
...
func convertName(tfname string) (module string, name string) {
	contract.Assertf(strings.HasPrefix(tfname, "readarr_"), "Invalid snake case name %s. Does not start with readarr_", tfname)
	tfname = strings.TrimPrefix(tfname, "readarr_")
...
func makeResource(res string) tokens.Type {
	...
	return tfbridge.MakeResource("readarr", mod, name)
}
