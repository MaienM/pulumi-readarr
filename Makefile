...
SHELL            := /bin/bash
PROJECT          := github.com/MaienM/pulumi-readarr
NODE_MODULE_NAME := @maienm/pulumi-readarr
TF_NAME          := readarr
...
TFGEN            := pulumi-tfgen-readarr
PROVIDER         := pulumi-resource-readarr
...
upstream/.git:
	...
	git -c advice.detachedHead=false clone --depth 1 --branch 'v$(PROVIDER_VERSION)' 'https://github.com/devopsarr/terraform-provider-readarr' upstream
